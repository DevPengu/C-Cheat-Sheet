...
## Arrays
Here's a chapter for **Arrays** formatted in the same style as the other sections:
...
int[] numbers = new int[5]; // An array of 5 integers this array get initialised with size 5, and 0 on all positions
string[] names = new string[] { "Alice", "Bob", "Charlie" }; // An array with initial values this array has initial values not default and size 3
```

**Explanation:** In C#, arrays are fixed in size, meaning you define their length at the time of creation and it cannot be changed. Each element in the array is accessible by its index, with the first element at index 0.
...
3. **Array Initialization with Default Values:** When you create an array without specifying values, C# initializes it with default values for that data type (e.g., `0` for `int`, `null` for `string`).

    ```csharp
    int[] numbers = new int[3]; // Initializes an int array of size 3 with values { 0, 0, 0 }
    ```
